...
export function MawkibOwnerRegisterPage() {
    ...
    <div className={`${guestTheme.page} flex justify-center p-4 py-8 sm:py-10`}>
      <PublicRegistrationForm
        ...
        onSubmit={async (payload) => {
          try {
            const data = await authApi.registerMawkibOwner(payload);
            setSession(data);
            navigate('/dashboard', { replace: true });
          } catch (err) {
            throw new Error(
              getApiErrorMessage(err, 'خطا در ثبت‌نام موکب‌دار. لطفاً دوباره تلاش کنید.'),
            );
          }
        }}
      />
