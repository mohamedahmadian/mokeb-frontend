...
function IconChevron() {
  return (
    <svg
      className="h-4 w-4 rotate-180 opacity-50"
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
      strokeWidth={2}
...
function ShowcaseSection({
  ...
  const sectionActions = actions ?? (cta ? [cta] : []);
  const bgClass =
    tone === "warm"
                ...
                <Link
                  ...
                  className={
                    action.variant === "secondary"
                      ? guestTheme.btnSecondary
                      : guestTheme.btnPrimary
                  }
                >
                  {action.label}
                </Link>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {quickServices.map((service) => (
              <Link
                key={service.to}
                to={service.to}
                className={guestTheme.serviceCard}
              >
                <div
